...
function validateGSTIN(e) {
    ...
    if (!gstin) {
        ...
        statusDiv.innerText = "GSTIN is required";
        return false;
    }
    ...
    // Optional: Regex check for alphanumeric + letters at specific positions
    const gstinRegex = /^[0-9A-Z]{15}$/; // simple: 15 chars, alphanumeric
    ...
    // Valid GSTIN
    statusDiv.style.display = "block";
    statusDiv.style.color = "#198754"; // green
...
// Optional: check on form submit
document.querySelector("form").addEventListener("submit", function (e) {
    const gstValid = validateGSTIN({ target: document.getElementById("gstin") });
    if (!gstValid) {
        e.preventDefault();
        alert("Please enter a valid 15-character GSTIN before submitting.");
    }
});
